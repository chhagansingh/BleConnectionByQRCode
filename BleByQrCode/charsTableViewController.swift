...
class charsTableViewController: UITableViewController , CBPeripheralDelegate, UITextFieldDelegate{
    ...
    func peripheral(_ peripheral: CBPeripheral, didUpdateValueFor characteristic: CBCharacteristic, error: Error?)
    {
        ...
        else
        {
            ...
            data.getBytes(&UpdateValue, length: MemoryLayout<Int>.size)


           // characteristic.value!.getBytes(&UpdateValue, length: sizeof(Int)) //Converts NSData object to Integer
            
            print("\nCharacteristic NSData: \(data)")
            // print("Characteristic Value string: \(characteristic.value!)")
             print("UpdateValue: \(UpdateValue)")
//            var notMuch: Int = 0
//            let notMuchNS = NSData(bytes: &notMuch, length: MemoryLayout<Int>.size)
            
            characteristicValue[characteristic.uuid] = characteristic.value! as NSData 
    ...
    func peripheral(_ peripheral: CBPeripheral, didUpdateValueFor desc: CBDescriptor, error: Error?)
    {
        ...
        else
        {
            ...
            //SHORT FORM if let r = desc.description.range(of: "Characteristic Format")
        ...
        if foundCharacteristics.count > 0
        {
            ...
            if foundCharacteristics[indexPath.row] != nil
            {
                ...
                if 0 != MyProperties & 4 || 0 != MyProperties & 8
                {
                    ...
                    {
                       // let myString = self.characteristicValue[Mycharacteristic.uuid.uuidString]!
                        let myString =  String(data: self.characteristicValue[Mycharacteristic .uuid]! as Data, encoding: String.Encoding(rawValue: String.Encoding.utf8.rawValue))
                    ...
                    cell.ValueEntryField.addTarget(self, action: #selector(self.newValue(sender:)), for: .editingDidEnd)
//                    cell.ValueEntryField.addTarget(self,action: #selector(charsTableViewController.newValue(_:)),for: UIControlEvents.EditingDidEnd)
                ...
                if 0 != MyProperties & 16 || 0 != MyProperties & 2 || 0 != MyProperties & 32
                {
                    ...
                    cell.Unsubscribe.addTarget(self,action:#selector(self.unSubscribe(sender:)),for: .touchUpInside)
                }
                ...
                if self.characteristicValue[Mycharacteristic.uuid] != nil
                {
                   ...
                   cell.rawValue.text =   String(data: self.characteristicValue[Mycharacteristic .uuid]! as Data, encoding: String.Encoding(rawValue: String.Encoding.utf8.rawValue))
                    
                    let data2: NSData =  characteristicValue[Mycharacteristic.uuid]! as NSData
                  
                    
                    data2.getBytes(&UpdateValue, length: MemoryLayout<Int>.size)
        ...
        
    {
        ...
        newValueNSD = NSData(bytes: &newValue, length: MemoryLayout<Int64>.size) //First let's set it to zero so we have SOMETHING
        ...
        else if myNSString.contains("0x") //But if it leads with 0x, let's scan for hex and send 32-bit hex value
        {
            if writeString.count < 11
            {
                ...
                newValueNSD = NSData(bytes: &anothernewValue, length: MemoryLayout<Int32>.size)
            }
            else
            {
                newvalScanner.scanHexInt64(&anothernewValue64)
                newValueNSD = NSData(bytes: &anothernewValue64, length: MemoryLayout<Int64>.size)
        ...
        else
        {
            ...
            newValueNSD = NSData(bytes: &newValue, length: MemoryLayout<Int32>.size)
        }
        ...
        {
            dummyValue = 1
            newValueNSD = NSData(bytes: &dummyValue, length: MemoryLayout<Int8>.size)
        }
